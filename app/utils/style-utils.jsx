                          ...
                          <div style={{ ...getSwatchStyle(isActive, settings, styleId, isCard), padding: isButton ? '8px 16px' : (isPillSwatch ? '6px 12px' : `${settings.basic.padding ?? settings.swatch?.padding ?? 0}px`) }}>
                              ...
                              {shouldShowName && (
                                  <div style={{ padding: isButton ? 0 : '5px', textAlign: 'center', width: '100%', maxWidth: '100%', lineHeight: '1.2', wordBreak: 'break-word', boxSizing: 'border-box' }}>
                                      ...
                                      {settings.price?.show && ( <div style={{ fontSize: `${settings.price?.fontSize || 10}px`, fontWeight: settings.price?.fontWeight || 'normal', color: settings.price?.color || '#6d7175' }}>{p.price}</div> )}
                                  </div>
                              )}
                          </div>
                      </div>
